...
public class SquidGrappleUtil {
    ...
    public static int onFireHook(LivingEntity entity, UUID newHookUUID) {
        ...
        if(tag.contains(indexStr)){
            EntitySquidGrapple hook = getHookEntity(entity.level(), tag.read(indexStr, net.minecraft.core.UUIDUtil.CODEC).orElse(null));
            if(hook != null && !hook.isRemoved()){
                hook.setWithdrawing(true);
            }
        }
        tag.store(indexStr, net.minecraft.core.UUIDUtil.CODEC, newHookUUID);
    ...
    public static int getAnyNullHooks(LivingEntity entity) {
        CompoundTag tag = CitadelEntityData.getOrCreateCitadelTag(entity);
        if (!tag.contains(HOOK_1) || getHookEntity(entity.level(), tag.read(HOOK_1, net.minecraft.core.UUIDUtil.CODEC).orElse(null)) == null) {
            return 0;
        }
        if (!tag.contains(HOOK_2) || getHookEntity(entity.level(), tag.read(HOOK_2, net.minecraft.core.UUIDUtil.CODEC).orElse(null)) == null) {
            return 1;
        }
        if (!tag.contains(HOOK_3) || getHookEntity(entity.level(), tag.read(HOOK_3, net.minecraft.core.UUIDUtil.CODEC).orElse(null)) == null) {
            return 2;
        }
        if (!tag.contains(HOOK_4) || getHookEntity(entity.level(), tag.read(HOOK_4, net.minecraft.core.UUIDUtil.CODEC).orElse(null)) == null) {
            return 3;
        }
        return -1;
    }
