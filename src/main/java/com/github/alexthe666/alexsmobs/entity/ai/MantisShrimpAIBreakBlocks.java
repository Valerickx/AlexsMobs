...
public class MantisShrimpAIBreakBlocks extends Goal {
    ...
    private void breakBlock() {
        if (shouldMoveTo(mantisShrimp.level(), destinationBlock)) {
            ...
            if(!mantisShrimp.level().isEmptyBlock(destinationBlock) && net.neoforged.neoforge.common.ForgeHooks.canEntityDestroy(mantisShrimp.level(), destinationBlock, mantisShrimp) && state.getDestroySpeed(mantisShrimp.level(), destinationBlock) >= 0){
