...
public class EntityCachalotPart extends PartEntity<EntityCachalotWhale> {
    ...
    public boolean hurtServer(ServerLevel level, DamageSource source, float amount) {
        if(this.level().isClientSide() && this.getParent() != null && !this.getParent().isInvulnerable()){
            Identifier key = source.typeHolder().unwrapKey().map(net.minecraft.resources.ResourceKey::identifier).orElse(null);
            if(key != null){
                AlexsMobs.sendMSGToServer(new MessageHurtMultipart(this.getId(), this.getParent().getId(), amount, key.toString()));
            }
        }
        return !this.isInvulnerable() && this.getParent().attackEntityPartFrom(this, source, amount);
    }
