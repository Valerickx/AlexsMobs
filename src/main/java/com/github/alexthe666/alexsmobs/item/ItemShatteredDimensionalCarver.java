...
public class ItemShatteredDimensionalCarver extends ItemDimensionalCarver {
    ...
    public void onPortalOpen(Level worldIn, LivingEntity player, EntityVoidPortal portal, Direction dir) {
        ...
        if (dir == Direction.DOWN) {
            portal.setDestination(new BlockPos(playerPos.getX(), worldIn.getMinY() + 1, playerPos.getZ()));
        } else if (dir == Direction.UP) {
            portal.setDestination(new BlockPos(playerPos.getX(), (worldIn.getMaxY() + 1) - 1, playerPos.getZ()));
        }else{
